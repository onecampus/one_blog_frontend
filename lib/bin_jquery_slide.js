...
(function ($){
		$.fn.binSlide=function (options){
				...
				return $(this).each(function (){
						var $this = $(this);
						var owrap=$(this).find(opt.img_box),//获取图片盒子
								object=owrap.find('.img'),//获取所有的图片
								oSilde=owrap.find('.slide'),//获取滑动标签
								wrapBtn=$(this).find(opt.btn_box),//获取按钮的外包元素
						...
						$this.AutoPlay=setInterval(next,opt.i_speed);
						...
						Button.hover(function(){clearInterval($this.AutoPlay);},function(){$this.AutoPlay = setInterval(next,opt.i_speed);});
				})
		};
})(jQuery);